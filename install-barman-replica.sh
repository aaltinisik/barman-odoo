...
echo -e "\n"
read -e -p "Enter password of postgres system user at $PG_IP : " PG_USERPASS
...
sshpass -p '$PG_USERPASS' ssh-copy-id -i ~/.ssh/id_rsa.pub postgres@$PG_IP

#ssh-copy-id -i ~/.ssh/id_rsa.pub postgres@$PG_IP

touch ~/.pgpass
chmod 600 ~/.pgpass
echo "$PG_IP:*:*:postgres:$POSTGRES_DBPASS" >> ~/.pgpass
